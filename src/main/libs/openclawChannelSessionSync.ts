 ...
 * to local Cowork sessions so that conversations are visible in the TopVanAI UI.
 */
...
export class OpenClawChannelSessionSync {
  ...
  resolveOrCreateSession(sessionKey: string): string | null {
    // 1. Skip TopVanAI-originated sessions
    if (isManagedSessionKey(sessionKey)) {
      console.log('[ChannelSessionSync] skipped: TopVanAI-originated session');
      return null;
    }
