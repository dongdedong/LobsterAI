  ...
  for (let index = 0; index < turns.length; index += 1) {
    ...
    if (assistantContent) {
      items.push({
        key: `${turn.id}-asst`,
        turnIndex: index,
        label: getRailLabel(assistantContent, 'TopVanAI'),
...
const composeExportCanvas = async (
  ...
  ctx.fillText('TopVanAI — 全场景个人助理 Agent', textX, footerCenterY - taglineFontSize / 2 - 2);

  ctx.fillStyle = subtitleColor;
  ctx.font = `400 ${taglineFontSize}px ${fontStack}`;
  ctx.fillText('7×24 小时帮你干活的全场景个人助理，由 TopVan 开发', textX, footerCenterY + brandFontSize / 2 + 3);
