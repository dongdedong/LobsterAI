...
<p align="center">
  <strong>7×24 小时帮你干活的全场景办公助手 Agent，由 TopVan 出品</strong>
</p>

<p align="center">
  <em>面向本地优先桌面 Agent 工作流的内部商业化分支。</em>
</p>
...
**TopVanAI** 是由 TopVan 出品的全场景办公助手 Agent。它 7×24 小时待命，真正帮你把活干完：数据分析、制作 PPT、生成视频、撰写文档、网络调研、收发邮件、定时任务，以及更多。

与只能聊天的助手不同，TopVanAI 是**桌面级** Agent。借助 **Cowork 模式**，它能直接连接你的文件、终端、浏览器与本地项目，在真实工作环境中执行工具、运行命令 —— 每一次敏感操作都需经你批准。你可以创建专用 Agent（股票研究、内容创作、备课出卷……），用技能与 MCP 服务持续扩展能力，还能通过已支持的 IM 连接器在手机上随时指挥电脑干活。
...
- **📱 手机远程指挥电脑干活** — 通过微信、企微、钉钉、飞书、QQ、Telegram、Discord 等已支持 IM 连接器远程指挥 TopVanAI
...
- **技能扩展** — 内置技能加自定义技能扩展；可用 `skill-creator` 自建技能并热加载
...
Vite 开发服务器默认运行在 `http://localhost:5175`。当前分支默认面向 Local BYOK MVP：用户自带模型供应商 API Key，首个商业化版本不依赖云端登录和模型代理链路。
...
该运行时以解释器为主，不预装 TopVanAI 技能所需的 Python 三方包；相关依赖可在运行时按需安装。
...
TopVanAI 采用 Electron 严格进程隔离架构，所有跨进程通信通过 IPC 完成。
...
- IM 网关 — 微信、企业微信、钉钉、飞书、QQ、Telegram、Discord 远程接入
- 40+ IPC 通道处理
...
Cowork 是 TopVanAI 的核心功能 —— 以 OpenClaw 为主引擎的 AI 工作会话系统。它面向办公场景设计，能够自主完成数据分析、文档生成、信息检索等复杂任务。
...
TopVanAI 内置很多 skill，覆盖办公、创作、投研、自动化等多种场景，通过 `SKILLs/skills.config.json` 配置启停和排序，以下是一些典型的 skill 案例：
...
| youdaonote | 有道云笔记遗留连接器 | 默认禁用；商业化使用前需单独复核服务依赖与授权 |
...
TopVanAI 支持创建定时任务，让 Agent 按计划自动执行重复性工作。
...
TopVanAI 支持将 Agent 桥接到多种 IM 平台。在手机上通过 IM 发送消息即可远程触发桌面端的 Agent 执行任务，随时随地指挥你的个人助理。
...
上游项目中的网易系 IM 连接器在当前分支中按遗留集成处理，不属于 Local BYOK MVP 的默认可见能力。商业化使用前需要单独完成依赖、授权和部署复核。

## 持久记忆

TopVanAI 的记忆系统基于 OpenClaw，以文件形式持久化存储在工作目录中，让 Agent 跨会话记住你的信息和偏好。
...
TopVanAI 将 OpenClaw 依赖锁定到指定的 release 版本，在 `package.json` 中声明：
