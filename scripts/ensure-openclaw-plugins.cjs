...
// Lightweight setup entry for deferred loading (patched by TopVanAI).
    ...
    if (!mediaSrc.includes(patchMarker)) {
      ...
      if (idx !== -1) {
        const replacement = `fileName = decodeURIComponent(match[1].trim());
                // Patched by TopVanAI: fix Latin-1 garbled UTF-8 filenames from Feishu API
        ...
        if (fnIdx !== -1) {
          const helperFn = `// Patched by TopVanAI: detect and fix Latin-1 garbled UTF-8 filenames.
...
`;
  ...
  if (fs.existsSync(dingtalkMsgHandlerPath)) {
    ...
    // TopVanAI writes platform-level agent bindings as accountId:"*" so one
