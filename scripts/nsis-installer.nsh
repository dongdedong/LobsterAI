...
;
; 1. TopVanAI.exe -- the main app AND the OpenClaw gateway (ELECTRON_RUN_AS_NODE)
; 2. node.exe whose binary lives inside the TopVanAI install tree
...
!macro stopTopVanAIProcesses
  DetailPrint "[Installer] Stopping running TopVanAI processes"
  System::Call 'kernel32::GetTickCount()i .r7'
  nsExec::ExecToLog 'powershell -NoProfile -NonInteractive -Command "\
    Stop-Process -Name TopVanAI -Force -ErrorAction SilentlyContinue;\
    Get-Process node -ErrorAction SilentlyContinue | Where-Object { $$_.Path -like \"*TopVanAI*\" } | Stop-Process -Force -ErrorAction SilentlyContinue;\
    for ($$i = 0; $$i -lt 15; $$i++) {\
      $$procs = @();\
      $$procs += Get-Process -Name TopVanAI -ErrorAction SilentlyContinue;\
      $$procs += Get-Process node -ErrorAction SilentlyContinue | Where-Object { $$_.Path -like \"*TopVanAI*\" };\
  ...
  IntOp $5 $6 - $7
  CreateDirectory "$APPDATA\TopVanAI"
  FileOpen $9 "$APPDATA\TopVanAI\install-timing.log" a
...
!macro customInit
  ...
  CreateDirectory "$APPDATA\TopVanAI"
  FileOpen $9 "$APPDATA\TopVanAI\install-timing.log" w
  !insertmacro GetTimestamp $8
...
!macro customCheckAppRunning
  !insertmacro stopTopVanAIProcesses

  !ifndef BUILD_UNINSTALLER
    ; -- Backup user-created skills to AppData before extraction overwrites them --
    ; Copy non-bundled skills to %APPDATA%\TopVanAI\skills-backup\ so they are
    ...
    System::Call 'kernel32::GetTickCount()i .r7'
    ClearErrors
    FileOpen $R0 "$APPDATA\TopVanAI\skill-migrate.log" w
    ...
    nsExec::ExecToStack 'powershell -NoProfile -NonInteractive -Command "\
      ...
      $$backup = \"$APPDATA\TopVanAI\skills-backup\";\
...
!macro customInstall
  ...
  ; Log file: %APPDATA%\TopVanAI\install-timing.log

  CreateDirectory "$APPDATA\TopVanAI"
  FileOpen $2 "$APPDATA\TopVanAI\install-timing.log" a
  !insertmacro GetTimestamp $8
  ...
  nsExec::ExecToLog '"$INSTDIR\${APP_EXECUTABLE_FILENAME}" "$INSTDIR\resources\unpack-cfmind.cjs" "$INSTDIR\resources\win-resources.tar" "$INSTDIR\resources" "$APPDATA\TopVanAI\install-timing.log"'
  Pop $0
  System::Call 'kernel32::GetTickCount()i .r6'
  IntOp $5 $6 - $7
  ...
  TarExtractProcessFailed:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "Resource extraction failed: could not start extractor process (exit=$0). This may be caused by antivirus software. See %APPDATA%\TopVanAI\install-timing.log for details."
    Goto TarExtractOK
  ...
  TarExtractNonZero:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "Resource extraction failed (exit code $0). See %APPDATA%\TopVanAI\install-timing.log for details."
  TarExtractOK:
  ...
  ; the backup.
  IfFileExists "$APPDATA\TopVanAI\skills-backup\*.*" 0 SkipSkillRestore
