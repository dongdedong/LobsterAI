...
> 生成日期: 2026-06-19
> 目标: 在不自建大模型推理、不自建图片/视频生成、不自建实时 ASR 的前提下，替换 TopVanAI 对有道网页和云服务的依赖，让产品进入“本地可用 + 用户自配模型 / 本地模型 + 自有轻量控制面”的可交付状态。
> 适用规模: 约 1000 个注册客户，100-300 日活，20-150 同时在线/同时运行任务的早期商业化规模。

## 1. 方案 A 的边界

方案 A 的核心判断是：TopVanAI 的 Agent 执行主链路并不必然需要云端运行。Electron 主进程、本地 SQLite、OpenClaw Gateway、用户自配 Provider、本地 Skills、Artifacts 预览都可以继续在用户机器上工作。服务端只承担轻量控制面和公共资源能力。
...
```mermaid
flowchart LR
  Client["TopVanAI Desktop"] --> Local["Local SQLite + OpenClaw Gateway"]
...
`E:\code\venusAI_Server` 已经实现了一套 TopVanAI 兼容后端雏形。它不是纯粹的方案 A，而是“方案 A + 极简方案 B 过渡层”：
...
1. 客户端短期可以少改。只要把 `getServerApiBaseUrl()` 配置到 VenusAI Server，就可以复用现有 TopVanAI 的 auth、server model、OpenClaw token proxy 路径。
...
1. **接口路径契约**：与 TopVanAI 当前主服务路径基本一致，可作为客户端改造的联调目标。
...
3. 用 TopVanAI 客户端配置到 `http://127.0.0.1:8787` 做一次手工联调。
...
6. `chore(docs): publish mirrored TopVanAI manual under self-hosted docs`
...
E:\code\TopVanAI> npx vitest run src\shared\remoteServices\constants.test.ts src\renderer\services\endpoints.test.ts
2 files passed, 10 tests passed

E:\code\TopVanAI> npx tsc --noEmit --pretty false
passed

E:\code\TopVanAI> npx tsc --project electron-tsconfig.json --noEmit --pretty false
passed

E:\code\TopVanAI> npx eslint <本轮触碰文件>
passed
```
...
E:\code\TopVanAI> npx vitest run src\shared\remoteServices\constants.test.ts src\renderer\services\endpoints.test.ts src\renderer\services\config.test.ts src\renderer\config.test.ts src\renderer\store\slices\asrQuotaSlice.test.ts
...
E:\code\TopVanAI> npx vitest run src\renderer\services\auth.test.ts src\shared\remoteServices\constants.test.ts src\renderer\services\endpoints.test.ts src\renderer\services\config.test.ts src\renderer\config.test.ts src\renderer\store\slices\asrQuotaSlice.test.ts
...
E:\code\TopVanAI> npx vitest run src\main\libs\endpoints.test.ts src\main\libs\openclawConfigSync.runtime.test.ts src\shared\remoteServices\constants.test.ts src\renderer\services\endpoints.test.ts src\renderer\config.test.ts src\renderer\services\config.test.ts src\renderer\services\auth.test.ts
