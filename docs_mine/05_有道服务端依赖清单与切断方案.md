...
|------|----------|-------------------|---------------|----------|
| P0 | LobsterAI 主业务服务 | 高 | `lobsterai-server.youdao.com` / `lobsterai-server.inner.youdao.com` | 必须替换为自有后端，或整体禁用登录、额度、服务端模型、媒体、ASR、分享 |
...
- 这是一个用户可选 LLM Provider，不等同于 LobsterAI 主业务服务。
...
| API Key 控制台 | `https://claw.163.com/projects/dashboard/?channel=LobsterAI#/api-keys` | `src/renderer/components/im/IMSettings.tsx` | UI 引导链接 |
...
| GitHub 仓库 | README | `github.com/netease-youdao/LobsterAI` |
| License | `LICENSE` | `Copyright (c) 2026 NetEase Youdao` |
